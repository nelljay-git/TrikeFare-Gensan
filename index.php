...
<body>
    ...
    <div id="app">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="sidebar-content">
                ...
                <div class="sidebar-item"
                    onclick="alert('Find Cab Route is still under development Please stay tuned!')">
        ...
        <div id="panel">
            ...
            <div class="app-header" onclick="togglePanel()">
                <div>
                    <span class="app-title">TrikeFare Gensan</span>
                ...
                <div class="modal-card">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button class="clear-history-btn" style="background: var(--primary-glow); color: var(--primary); border: 1px solid var(--primary-glow);" onclick="fetchHistoryFromServer()" title="Reload History"><i class="fa-solid fa-arrows-rotate"></i> Reload</button>
                            <button class="clear-history-btn" style="background: var(--primary-glow); color: var(--primary); border: 1px solid var(--primary-glow);" onclick="syncHistoryToServer()"><i class="fa-solid fa-cloud-arrow-up"></i> Sync</button>
                            <button class="clear-history-btn" id="clearHistBtn" onclick="clearHistory()"><i class="fa-solid fa-trash"></i> Clear</button>
                        </div>
                    </div>
    ...
    <div class="modal-overlay" id="rideDetectionModalOverlay" style="z-index: 100000;">
        <div class="fare-modal"
            style="max-width: 320px; border: 2px solid var(--primary); box-shadow: 0 0 30px rgba(0, 212, 170, 0.3);">
            <div class="modal-header">
                <div>
                    <div class="modal-title" style="color: white;"><i class="fa-solid fa-motorcycle"></i> Ride Detected!
                    </div>
                    <div class="modal-subtext" style="color: rgba(255,255,255,0.8);">We noticed you're moving</div>
                </div>
                <button class="btn-close" onclick="closeRideDetectionModal()"
                    style="color: white; opacity: 0.8;">×</button>
            </div>
            <div class="modal-content" style="text-align: center; padding: 25px 20px;">
                <div class="ride-detect-icon-wrapper" style="margin-bottom: 20px;">
                    <div style="font-size: 3.5rem; color: var(--primary); animation: pulse-primary 2s infinite;">
                        <i class="fa-solid fa-person-biking"></i>
                    </div>
                </div>
                <h3 style="margin-bottom: 12px; font-size: 1.2rem; font-weight: 800; color: var(--text);">Are you riding
                    now?</h3>
                <p style="font-size: 0.9rem; color: var(--text-dim); margin-bottom: 25px; line-height: 1.5;">
                    Don't forget to start ride tracking to get an accurate fare estimate for your journey!
                </p>
    ...
    <script>
        ...
        function closeRideDetectionModal() {
            ...
            untrackedDistance = 0; // Reset after modal is handled
        }
      ...
      `;
        ...
        function onPosition(pos) {
            ...
            if (lastPos) {
                ...
                } else if (!rideDetectionModalShown) {
                    // Ride Detection Logic: Check if user moved >= 150m and speed >= 3km/h
                    untrackedDistance += distKm;
                    if (untrackedDistance >= 0.150 && speedKmh >= 3) {
                        ...
                        const html = `
                            ...
                            <div id="topFare" class="premium-suggestion-card" style="margin-top: 8px; border: 1px solid var(--warning-glow); background: rgba(240, 165, 0, 0.03);">
                                <div class="suggestion-main-wrapper">
                                    ...
                                    <div class="suggestion-stats" style="margin-top: 6px; padding-top: 6px;">
                                        <div class="trust-score" style="color: ${ratingColor}" id="score-${topFare.id}">
                                            <i class="fa-solid fa-thumbs-up"></i> ${votes} votes
                                        </div>
                                        <div class="rating-controls" style="gap:4px;">
                                            <button class="rate-btn up ${tfUpClass}" onclick="rateFare(${topFare.id}, 'upvote', this)" title="Upvote"><i class="fa-solid fa-thumbs-up"></i></button>
                                            <button class="rate-btn down ${tfDownClass}" onclick="rateFare(${topFare.id}, 'downvote', this)" title="Downvote"><i class="fa-solid fa-thumbs-down"></i></button>
        ...
        async function submitCommunityFare() {
            ...
            if (!navigator.onLine) {
                ...
                if (awaitingFareSubmission && lastCompletedRideData) {
                    ...
                    setStatus('Stored offline & saved', 'warning');
                    resetRide();
            ...
            try {
                ...
                if (res.status === 401 || res.status === 429 || res.status >= 500) {
                    ...
                    const errMsg = errData.error || (res.status >= 500 ? 'Server Error: Unable to submit fare.' : 'Access Denied');
                    showToast(errMsg, 'error');
                    return;
                }

                const result = await res.json();
                finalizeFareSubmission(result, payload);
                ...
                if (!navigator.onLine || err.name === 'TypeError') {
                    ...
                    showToast('You are offline. Fare saved locally and will upload automatically when online.', 'info', 5000);

                    // Finalize UI and save to local history
                    closeSubmitFareModal();
                    rideJustCompleted = false;
                    if (awaitingFareSubmission && lastCompletedRideData) {
                        saveRide(lastCompletedRideData.dist, lastCompletedRideData.fare, lastCompletedRideData.duration);
        ...
        function finalizeFareSubmission(result, payload) {
            if (result.success) {
                // Full form reset
                document.getElementById('submitFareAmount').value = '';
                document.getElementById('submitOrigin').value = '';
                document.getElementById('submitDestination').value = '';
                const noteEl = document.getElementById('submitFareNote');
                if (noteEl) { noteEl.value = ''; }
                const noteCountEl = document.getElementById('submitFareNoteCount');
                if (noteCountEl) noteCountEl.textContent = '0/100';
                ...
                if (lastDestination) fetchCommunityFares();

                if (result.is_vote) {
                    const votes = JSON.parse(localStorage.getItem('trikefareVotes') || '{}');
                    votes[result.id] = 'upvote';
                    localStorage.setItem('trikefareVotes', JSON.stringify(votes));
                }
                showToast(result.message || 'Fare submitted! Thank you for contributing.', 'success');
            } else {
                showToast(result.error || 'Failed to submit fare.', 'error');
            }
        }
        ...
        // Initial sync check (handled by offline_sync.js)
        ...
        function renderExplorerList() {
            ...
            list.innerHTML = subs.map(item => {
                ...
                const votedLabel = existingVote ? `<div class="vote-status">${existingVote === 'upvote' ? '<i class="fa-solid fa-thumbs-up"></i>' : '<i class="fa-solid fa-thumbs-down"></i>'} Voted</div>` : '';
